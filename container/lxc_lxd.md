...
* 特定の IP アドレスに対して VPN ルーター経由でアクセスする  
  (例) Mac 上の VirtuablBox ゲストにコンテナを設置し、ゲストの仮想 NIC に NAT とブリッジネットワークの 2種類を割り当てている場合。
  ...
  1. ゲストOS側では NAT 経由で出て行くようスタティックルーティングを設定  
     `sudo ip r add <目的のIP/サブネット> via <NAT用NICのIP>`  
     (例) NATネットワークの NIC の IP が 10.1.2.1 の場合  
