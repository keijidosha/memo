...
(2025.08.31 追記) ⬇︎フォルダコピーの方法で別ドライブに移動すると、次のエラーが発生。  
```
xxx.vbox"を開けませんでした。
呼び出し先 RC:
NS_ERROR_INVALID_ARG (0X80070057)
```
そこで 次の方法で移動するのがおすすめになりそう。  
1. Virtual Box の GUI 画面から、移動したい VM を右クリック。
1. 「移動...」をクリックすると、フォルダ指定のダイアログが開くので、移動先ドライブ/フォルダを選択。 

※ゲスト OS が Windows の場合など、[設定] - [ストレージ] で Unattended-<uuid>-aux-iso.viso というファイルが参照されていると移動できない模様。  
　その場合は Unattended-<uuid>-aux-iso.viso をデタッチ(割り当てを除去)すると、移動できるようになった。
